...
verify_fusion.py
================
Verification script for Phase 6 C++ — Fusion node.

Checks:
  1. Build artefacts present (fusion_node_cpp binary, calib_utils library)
  2. GTests pass (test_projector + test_fusion_engine — 19 tests total)
  3. ROS dry-run: node starts cleanly with calibration_file param

Run with:
  pixi run verify_fusion
"""
...
CALIB     = os.path.join(ROOT, "src", "perception_pipeline", "config", "calibration.yaml")
...
try:
    ...
    # Count how many test suites ran
    import re
    ...
    check(f"{pct}% tests passed ({total} suites)", all_passed,
          result.stdout[-300:] if not all_passed else "")
except FileNotFoundError:
    check("colcon available", False, "colcon not on PATH — run inside pixi shell")
except subprocess.TimeoutExpired:
    check("colcon test", False, "timed out after 90 s")
...
try:
    ...
    check("node prints 'FusionNode ready'", ready,
          output[-400:] if not ready else "")
...
print()
if errors == 0:
    print("\033[32mAll checks passed — Phase 6 (fusion node) verified.\033[0m")
    sys.exit(0)
else:
